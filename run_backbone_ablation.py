...
def process_dataset(dataset, classes, config_path, results_dir):
    ...
    csv_path = os.path.join(results_dir, f'backbone_ablation_{dataset}.csv')
    txt_path = os.path.join(results_dir, f'backbone_summary_{dataset}.txt')
    ...
    print(f"\n[INFO] Backbone ablation completed! Summary saved to: {txt_path}")
...
if __name__ == '__main__':
    seed_everything(42)
    process_dataset('mvtec', MVTEC_CLASSES, 'NetworkConfigs/s2ad_configs/MVTec.yaml', './results_backbone_ablation')
