...
class ESVAE(nn.Module):
    def __init__(self, device, distance_lambda, mmd_type):
        ...
        input_size = glv.network_config.get('input_size', 32)
        downsampled_size = input_size // (2 ** len(hidden_dims))
        self.downsampled_size = downsampled_size
        flattened_size = hidden_dims[-1] * (downsampled_size ** 2)

        self.before_latent_layer = tdLinear(flattened_size,
                                            latent_dim,
                                            bias=True,
    ...
    def decode(self, z):
        ...
        result = result.view(result.shape[0], self.hidden_dims[-1], getattr(self, 'downsampled_size', 2), getattr(self, 'downsampled_size', 2), self.n_steps)  # (N,C,H,W,T)
...
class ESVAELarge(ESVAE):
    def __init__(self, device, distance_lambda, mmd_type):
        ...
        input_size = glv.network_config.get('input_size', 64)
        downsampled_size = input_size // (2 ** len(hidden_dims))
        self.downsampled_size = downsampled_size
        flattened_size = hidden_dims[-1] * (downsampled_size ** 2)
